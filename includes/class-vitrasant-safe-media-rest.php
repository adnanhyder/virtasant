...
class Virtasant_Safe_Media_Rest
{
    ...
    public function register_routes()
    {
        ...
        register_rest_route(
            ...
            array(
                ...
                array(
                    'methods' => 'GET',
                    'callback' => array($this, 'return_image'),
                    'permission_callback' => '__return_true',
                ),
            )
        );
    ...
    public function return_image()
    {
        ...
        foreach ($ids as $single_id) {
            $data_array[] = $this->retrun_attached_obj($single_id);
        }
    ...
    public function retrun_attached_obj($id)
    {
        $return_array = [];
        $msg = "";
        $msg .= $this->safe_media->vitrasant_prevent_content_image_deletion($id);
        $msg .= " ";

        $msg .= $this->safe_media->vitrasant_prevent_featured_image_deletion($id);
        $msg .= " ";

        $msg .= $this->safe_media->vitrasant_prevent_term_image_deletion($id);
        $msg .= " ";
        $return_array[$id] = $msg;
